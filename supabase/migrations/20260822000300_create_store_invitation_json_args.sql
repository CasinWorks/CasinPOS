-- Recreate create_store_invitation with JSON-friendly args.
-- PostgREST was 404ing the live web client on the enum/uuid[] signature
-- (PGRST202 schema-cache miss). text + jsonb match Flutter JSON bodies.

drop function if exists public.create_store_invitation(uuid, text, public.store_role, uuid[]);
drop function if exists public.create_store_invitation(uuid, text, public.store_role);
...
begin
  ...
  if v_row.id is null then
    insert into public.store_invitations (
      ...
      p_store_id, v_email, v_role, v_valid_branches, auth.uid()
    )
    returning * into v_row;
  else
    v_resent := true;
